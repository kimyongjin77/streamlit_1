...
def main():
    ...
    if choice==menu[0]:
        ...
        #accept_multiple_files=True로 셋팅
        #여러 파일들을 한꺼번에 받을 수 있다.
        upload_files=st.file_uploader('이미지파일선택',type=['jpg','png','jpeg'],accept_multiple_files=True)
        #upload_files는 여러파일들을 저장하고 있는 리스트
        #업로드한 파일이 있는 경우에만, 아래 코드를 실행한다.
        if upload_files is not None:
            ...
            for file in upload_files:
                ...
                #현재시간을 활용해서, 파일명을 만든다, 파일명에 ':'는 사용할 수 없다.
            ...
            #저장이 끝나면 , 웹브라우져에 이미지 표시
            for file in upload_files:
                img=Image.open(file)
                st.image(img)
    
    ##CSV파일을 여러개 올리면, 이 파일들을 temp에 저장하고
    ##데이터프레임으로 읽어서 화면에 표시
    elif choice==menu[1]:
        st.subheader('CSV파일 업로드')
        upload_files=st.file_uploader('CSV파일선택',type=['csv'],accept_multiple_files=True)
        if upload_files is not None:
            for file in upload_files:
                #파일명을 유니크하게 만들어서 저장해야 한다.
                #현재시간을 활용해서, 파일명을 만든다, 파일명에 ':'는 사용할 수 없다.
            ...
            for file in upload_files:
                st.text(file.name)
                #st.dataframe(pd.read_csv('temp/' + file.name))
